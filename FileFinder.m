...
classdef FileFinder
    ...
    methods (Static)
        ...
        function pathsList = findAllFilePathsInDirectoryEndingWithSuffixRecursive(directory, suffix)
            filePathsList = FileFinder.getAllFilesRecursive(directory);
            paths = FileFinder.pathsForFilePathsList(filePathsList, suffix);
        ...
        function fileList = getAllFilesRecursive(dirName)
            dirData = dir(dirName);      %# Get the data for the current directory
            dirIndex = [dirData.isdir];  %# Find the index for directories
            fileList = {dirData(~dirIndex).name}';  %'# Get a list of the files
        ...
        function paths = pathsForFilePathsList(filePathsList, suffix)
            ...
            for i=1:size(filePathsList,1)
                ...
                if ~isempty(findstr(filePath{1}, suffix)) && FileFinder.doesStringEndWithString(filePath{1},suffix)
        ...
        function doesEnd = doesStringEndWithString(string, suffix)
            sl = length(string);
            pl = length(suffix);

            doesEnd = (sl >= pl && strcmp(string(sl-pl+1:sl), suffix)) || isempty(suffix);
        end
    end
end
